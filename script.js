document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveState() {
        try {
            localStorage.setItem('plannerTags', JSON.stringify(tags));
            localStorage.setItem('plannerDateData', JSON.stringify(dateData));
            localStorage.setItem('plannerCounterVisible', counterToggle.checked);
            localStorage.setItem('plannerGridColumns', gridSizeSlider.value);
        } catch (e) { console.error("Failed to save state:", e); }
    }

    function loadState() {
        const savedTags = localStorage.getItem('plannerTags');
        const savedDateData = localStorage.getItem('plannerDateData');
        const counterVisible = localStorage.getItem('plannerCounterVisible');
        const savedGridColumns = localStorage.getItem('plannerGridColumns');

        tags = savedTags ? JSON.parse(savedTags) : [ { name: 'Holiday', color: '#e74c3c' }, { name: 'Absent', color: '#f1c40f' } ];
        dateData = savedDateData ? JSON.parse(savedDateData) : {};
        counterToggle.checked = counterVisible !== null ? JSON.parse(counterVisible) : true;
        
        if (savedGridColumns) {
            gridSizeSlider.value = savedGridColumns;
            gridSizeValue.textContent = savedGridColumns;
            calendarContainer.style.gridTemplateColumns = `repeat(${savedGridColumns}, 1fr)`;
        }
    ...
    function setDefaultDates() {
        const twoMonthsLater = new Date(localToday);
    ...
    function bindEvents() {
        ...
        gridSizeSlider.addEventListener('input', (e) => {
            ...
            gridSizeValue.textContent = columns;
            calendarContainer.style.gridTemplateColumns = `repeat(${columns}, 1fr)`;
        });
    ...
    // --- NEW: Confetti shower function ---
    function triggerConfettiShower() {
        const duration = 2 * 1000; // 2 seconds
        const animationEnd = Date.now() + duration;
        const colors = ['#FFFFFF', '#FFC0CB', '#FFD700', getComputedStyle(document.documentElement).getPropertyValue('--accent-color')];
